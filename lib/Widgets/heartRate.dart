...
class heartRateGraph extends StatelessWidget {
  ...
  heartRateGraph(this.heartRateData, this.maxX, this.minX, this.maxY, this.minY, this.avgHeartRate);
  ...
  Widget build(BuildContext context) {
    return
      ...
      children: <Widget>[
         Text(
                'Your HeartRate: ${avgHeartRate.toStringAsFixed(1)} Beats per Min',
  ...
  List<FlSpot> _generateDataPoints() {
     for (var heartRate in heartRateData!) {
      dataPoints.add(FlSpot(heartRate.dateFrom.millisecondsSinceEpoch.toDouble(), double.parse(heartRate.value.toString()).toDouble()));
     }
    return dataPoints;
  }
