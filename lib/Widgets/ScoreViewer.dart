...
class ScoreViewWidget extends StatelessWidget {
  ...
  final bool stepsReady;
    //List<Widget> sleepDisplay; 


  late final List<Widget> scoreViews = [  //snippet to add sleep chart : sleep: sleepDisplay[0]
    ScoreCard(request: request, title: "Sunday", date: sunday, openStats: openStats, stepsReady: stepsReady, weekday: 0), 
    ScoreCard(request: request, title: "Monday", date: sunday.add(const Duration(days: 1)), openStats: openStats, stepsReady: stepsReady, weekday: 1),
    ScoreCard(request: request, title: "Tuesday", date: sunday.add(const Duration(days: 2)), openStats: openStats, stepsReady: stepsReady, weekday: 2), 
    ScoreCard(request: request, title: "Wednesday", date: sunday.add(const Duration(days: 3)), openStats: openStats, stepsReady: stepsReady, weekday: 3),
    ScoreCard(request: request, title: "Thursday", date: sunday.add(const Duration(days: 4)), openStats: openStats, stepsReady: stepsReady, weekday: 4), 
    ScoreCard(request: request, title: "Friday", date: sunday.add(const Duration(days: 5)), openStats: openStats, stepsReady: stepsReady, weekday: 5),
    ScoreCard(request: request, title: "Saturday", date: sunday.add(const Duration(days: 6)), openStats: openStats, stepsReady: stepsReady, weekday: 6),
    ];
                    ...
                    Padding(
                      padding: const EdgeInsets.only(left: 30, bottom: 5),
                       // child: SafeArea(child:  request.sleepCharts[weekday])
                        child:  SafeArea(child: stepsReady ? CardSleepGraph(request.sleepPoints, request.maxs[weekday], request.mins[weekday], false) : const SizedBox(width: 280,child: Center(child: CircularProgressIndicator(),)))
